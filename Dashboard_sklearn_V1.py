...
def plot_corr_glow():
    ...
    load_time=[]
    for i in range(len(refined_df_glow)):
        if refined_df_glow['PVD MACHINE LOADING TIME'][i]!=None and type(refined_df_glow['PVD MACHINE LOADING TIME'][i])!=float and ":" not in refined_df_glow['PVD MACHINE LOADING TIME'][i] and "#" not in refined_df_glow['PVD MACHINE LOADING TIME'][i] and refined_df_glow['PVD MACHINE LOADING TIME'][i]!='0':
...
def plot_trend_glow():
    ...
    v_tot_line = base.mark_line(color="#808081").encode(
        y=alt.Y('Voltage Integral:Q', title='Voltage Integral', scale=alt.Scale(domain=[75000, 75600])),
    ...
    # Combine all layers and display
    chart = alt.layer(v_tot_line, text, v_mv_line,v_scatter)     
...
if st.session_state.current_page == 'main':
    ...
    st.markdown('<div class="main-heading">PVD Dashboard V1.0 - 30Aug24 </div>', unsafe_allow_html=True)
