...
def individual_plot_ac(sample_no):
    ...
    golden_current_values=golden_ac_process['BIAS CURRENT ACTUAL'].values
    ...
    c1_line = base.mark_line(color="#40d081").encode(
        y=alt.Y('Arc 1 Current:Q'),
    ...
    c2_line = base.mark_line(color="#40d081").encode(
        y=alt.Y('Arc 2 Current:Q'),
    ...
    c3_line = base.mark_line(color="#40d081").encode(
        y=alt.Y('Arc 3 Current:Q'),
    ...
    #plots=[[v_line,golden_v_line],[c_line,golden_c_line],[g_line,golden_g_line],[vacuum_line,golden_vacuum_line],[t_line,golden_t_line],[tvp_line,golden_tvp_line]]
    plots=[v_line,c1_line,c2_line,c3_line,g_line,vacuum_line,t_line,tvp_line]
...
def individual_plot_ae(sample_no):
    ...
    idxs=refined_df_ae['Recipe Idxs']
    ...
    golden_current_values=golden_ae_process['BIAS CURRENT ACTUAL'].values
    ...
    empty_line = base.mark_line(color="black").encode(
        ...
        title='Arc Cleaning: Batch ' + refined_df_ae['BATCH ID'][sample_no] + "  " + str(st.session_state.df_ae['DATE TIME'][refined_df_ae['Idxs'][sample_no][0]]),
        width=600,
        height=400
    )
    v_line = base.mark_line(color="#00a0a1").encode(
    ...
    with col2:
        current1 = st.checkbox("Current 1",value=True)
    with col3:
        current2 = st.checkbox("Current 2",value=True)
    with col4:
        current3 = st.checkbox("Current 3 ",value=True)
    with col5:
        ar_gas = st.checkbox("Argon",value=True)
    with col6:
        vacuum = st.checkbox("Vacuum",value=True)
    with col7:
        temperature = st.checkbox("Temperature",value=True)
    with col8:
        tvp = st.checkbox("TVP",value=True)
    chart=empty_line
    inps=[bias_voltage,current1,current2,current3,ar_gas,vacuum,temperature,tvp]
    #plots=[[v_line,golden_v_line],[c_line,golden_c_line],[g_line,golden_g_line],[vacuum_line,golden_vacuum_line],[t_line,golden_t_line],[tvp_line,golden_tvp_line]]
    plots=[v_line,c1_line,c2_line,c3_line,g_line,vacuum_line,t_line,tvp_line]
...
def plot_trend_ae():
    ...
    c1_tot_line = base.mark_line(color="#808081").encode(
        y=alt.Y('Arc 1 Current Integral:Q', title='Arc 1 Current Integral', scale=alt.Scale(domain=[0, 55000])),
        tooltip=['Date:N', 'Arc 1 Current Integral:Q','BATCH ID:N']
    ).properties(
        title='Trend of Arc 1 Current Integral',
        width=600,
    ...
    c2_tot_line = base.mark_line(color="#808081").encode(
        y=alt.Y('Arc 2 Current Integral:Q', title='Arc 2 Current Integral', scale=alt.Scale(domain=[0, 65000])),
        tooltip=['Date:N', 'Arc 2 Current Integral:Q','BATCH ID:N']
    ).properties(
        title='Trend of Arc 2 Current Integral',
        width=600,
    ...
    st.altair_chart(chart, use_container_width=True)
    c3_tot_line = base.mark_line(color="#808081").encode(
        y=alt.Y('Arc 3 Current Integral:Q', title='Arc 3 Current Integral', scale=alt.Scale(domain=[0, 55000])),
        tooltip=['Date:N', 'Arc 3 Current Integral:Q','BATCH ID:N']
    ).properties(
        title='Trend of Arc 3 Current Integral',
        width=600,
    ...
    # Line chart
    v_tot_line = base.mark_line(color="#808081").encode(
        y=alt.Y('Voltage Integral:Q', title='Voltage Integral', scale=alt.Scale(domain=[50000, 120000])),
        tooltip=['Date:N', 'Voltage Integral:Q','BATCH ID:N']
    ).properties(
        title='Trend of Voltage Integral',
        width=600,
...
def plot_coating_type_glow():
    ...
    chart = (bars).properties(
        title='Coating Type vs Glow Discharge Batches',
        width=600,
        height=400
    )
...
def plot_coating_type_ac():
    ...
    chart = (bars).properties(
        title='Coating Type vs Arc Cleaning Batches',
        width=600,
        height=400
    )
...
def compute_glow(process_data_glow,io_data_glow):
    ...
    golden_glow_process=df_glow[v_time_id[golden_index_glow][0]:v_time_id[golden_index_glow][1]]
...
def compute_ac(process_data_ac,io_data_ac):
    global df_ac
    ...
    #st.session_state.start_date=refined_df_ac['BATCH START TIME'][0][:10]
    #st.session_state.end_date=refined_df_ac['BATCH START TIME'][0][:10]
    a=refined_df_ac
    a['BATCH START TIME']=pd.to_datetime(a['BATCH START TIME'], format='%d/%m/%Y %H:%M:%S')
    a = a.sort_values(by='BATCH START TIME')
    a = a.reset_index(drop=True)
    ...
    golden_ac_process=df_ac[step2_idxs[golden_index_ac][0]:step2_idxs[golden_index_ac][1]]
...
def compute_ae(process_data_ae,io_data_ae):
    global df_ae
    ...
    column_headers=["DATE TIME","BATCH ID", "Idxs","Recipe","Recipe Idxs","V Irregulars","Bias Arc Irregulars","I1 Irregulars","I2 Irregulars","I3 Irregulars","Ar Gas Irregulars","V total","I1 total","I2 total","I3 total","Arc total","Ar Gas total"]
    ...
    for i in range(len(idxs_ae)):
        ...
        batch_data_ae.loc[i, 'Idxs'] = idxs_ae[i]
        batch_data_ae.loc[i, 'Recipe'] = recipe[i]
        batch_data_ae.loc[i, 'Recipe Idxs'] = rec_idxs[i]
    ...
    merged_df_ae = pd.merge(batch_data_ae[columns_to_select],in_df, on="BATCH ID", how="inner")
    ...
    #st.session_state.start_date=refined_df_ae['BATCH START TIME'][0][:10]
    #st.session_state.end_date=refined_df_ae['BATCH START TIME'][0][:10]
    a=refined_df_ae
    a['BATCH START TIME']=pd.to_datetime(a['BATCH START TIME'], format='%d/%m/%Y %H:%M:%S')
    a = a.sort_values(by='BATCH START TIME')
    a = a.reset_index(drop=True)
    ...
    golden_ae_process=df_ae[rec_idxs[golden_index_ae][0]:rec_idxs[golden_index_ae][1]]
    ...
    region_selections_ae = [True, False, False, False,False,False, False]
...
def change_date_ac(start_date_ac,end_date_ac,region_selections_ac):
    ...
    if region_selections_ac[0]==True:
        ...
        if region_selections_ac[3]==True:
            comb_df=pd.concat([comb_df,filtered_df[filtered_df["I1 Irregulars"]==True]], ignore_index=True)
        if region_selections_ac[4]==True:
            comb_df=pd.concat([comb_df,filtered_df[filtered_df["I2 Irregulars"]==True]], ignore_index=True)
        if region_selections_ac[5]==True:
            comb_df=pd.concat([comb_df,filtered_df[filtered_df["I3 Irregulars"]==True]], ignore_index=True)
        if region_selections_ac[6]==True:
            comb_df=pd.concat([comb_df,filtered_df[filtered_df["Ar Gas Irregulars"]==True]], ignore_index=True)      
        filtered_df=comb_df.drop_duplicates(subset=['BATCH ID'])
        filtered_df=filtered_df.reset_index(drop=True)
...
if st.session_state.current_page == 'main':
    ...
    if st.button("Submit"):
        ...
        print(len(st.session_state.process_data_ac))
    ...
    st.markdown('<div class="main-heading">PVD Dashboard V2.0 - 06Sep24 </div>', unsafe_allow_html=True)
    ...
    with col4:
        if st.button("ARC CLEANING"):
            st.session_state.current_page='ac main'
            st.rerun()  
    with col5:
        if st.button("ARC ETCHING"):
            st.session_state.current_page='ae main'
            st.rerun()  
    ...
    region_selections = [False,False,False,False,False,False,False,False]
    # If 'Irregular Batches' is selected, show checkboxes for irregularities
    ...
    if batch_type == "Irregular Batches":
        ...
        region_selections = [recipe_type, batch_type, voltage_irreg, arc_irreg, current1_irreg, current2_irreg, current3_irreg, ar_gas_irreg]
...
# Display the plot in Streamlit
    ...
    if st.sidebar.button("Back"):
        st.session_state.current_page = 'ae_overview'
        st.rerun()
    batch_options = [f"{st.session_state.df_ae['BATCH ID'][i[0]]}  {st.session_state.df_ae['DATE TIME'][i[0]]}" for i in st.session_state.refined_df_ae['Idxs']]
    selected_batch = st.sidebar.radio("Select a Batch", options=batch_options, index=0)
