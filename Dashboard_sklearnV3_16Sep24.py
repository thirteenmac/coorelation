...
def individual_plot_ae(sample_no):
    ...
    empty_line = base.mark_line(color="black").encode(
        ...
        title='Arc Etching: Batch ' + refined_df_ae['BATCH ID'][sample_no] + "  " + str(st.session_state.df_ae['DATE TIME'][refined_df_ae['Idxs'][sample_no][0]]),
        width=600,
        height=400
    )
    v_line = base.mark_line(color="#00a0a1").encode(
...
def compute_ae(add_data,process_data_ae,io_data_ae):
    ...
    if add_data==True:
        ...
        columns_to_select = ["Idxs","Recipe Idxs"]

        for col in columns_to_select:
            for i in range(len(refined_df_ae)):
                refined_df_ae[col][i][0]=refined_df_ae[col][i][0]+new_start_index
                refined_df_ae[col][i][1]=refined_df_ae[col][i][1]+new_start_index
    ...
    for i in range(len(refined_df_ae)):
        print(list(refined_df_ae['Idxs'])[i],list(refined_df_ae["Recipe Idxs"])[i])
...
if st.session_state.current_page == 'computing':
    ...
    if st.button("Submit"):
        ...
        if uploaded_zip is not None:
            ...
            import os
            extract_path = "extracted_folder"  # Local path to extract the zip contents
